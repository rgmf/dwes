...
def generate_list_number(**kwargs: dict[str, int]) -> list[int]:
    ...
    """
    start: int | None = kwargs["start"] if "start" in kwargs else None
    end: int | None = kwargs["end"] if "end" in kwargs else None
...
def conditional_op(operation: str, *args: tuple, **kwargs: dict) -> float:
    ...
    return sum(numbers) if operation == ADD else sum(numbers) / len(numbers)
