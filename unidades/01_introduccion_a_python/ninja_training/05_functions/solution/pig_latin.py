...
def pig_latin(sentence: str) -> str:
    ...
    pig_latin_words: list[str] = []
    for w in words:
        pig_latin_words.append(pig_latin_word(w))
...
def pig_latin_word(word: str) -> str:
    ...
    idx_first_vowel: int = get_index_first_vowel(word)
    pig_latin_word: str = word[idx_first_vowel:] + word[0:idx_first_vowel] + "ay"

    return pig_latin_word.capitalize() if word[0].isupper() else pig_latin_word


def get_index_first_vowel(word: str) -> str:
