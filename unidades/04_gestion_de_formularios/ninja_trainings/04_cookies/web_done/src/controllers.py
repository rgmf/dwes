...
def home(environ: dict[str, list[str]]) -> str:
    ...
    if "name" in cookies and "visited" in cookies:
        name: str = cookies["name"].value
        visited: str = cookies["visited"].value
        context: dict[str, str] = {
            "info": f"Eres {name} y viniste aquí por útima vez el día {visited}"
        }
    else:
        context: dict[str, str] = {
            "info": render_template("src/views/parts/who_are_you_link.html")
...
def yourname(environ: dict[str, list[str]], headers: list[tuple[str, str]]) -> str:
    ...
    headers.append(("Set-Cookie", f"name={name}"))
    headers.append(("Set-Cookie", f"visited={dt}"))
